...
public class PhysicsSystem2D {
    ...
    private void applyImpulse(RigidBody2D a, RigidBody2D b, CollisionManifold m) {
        ...
        a.setLinearVelocity(new Vector2(a.getLinearVelocity())
                .add(new Vector2(impulse).multiply(invMass_A).multiply(-1)));

        b.setLinearVelocity(new Vector2(b.getLinearVelocity())
                .add(new Vector2(impulse).multiply(invMass_B).multiply(1)));
    }
